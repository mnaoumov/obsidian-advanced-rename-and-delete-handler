  ...
  it('is left alone by this plugin, rename by rename', async () => {
    const result = await evalInObsidian({
      async callback({
         ...
         * the wait that actually overran. A helper's budget is charged once per CALL SITE, and this one has
         * FIVE — `runPhase` runs twice and each run both applies settings and waits for the cache, plus the
         * `applySettings` in the `finally` — for 25 000 ms in total. That multiplier is why the number is so
         * much smaller than the `desktop` project's own raised `commandTimeoutInMilliseconds`: the budget is
         * kept under the transport's DEFAULT cap so it means the same thing wherever this closure runs. What
         * is waited on here lands in well under a second, so a budget this size costs nothing — a wait that
         * can genuinely run long belongs in `pollInObsidian`, with Node doing the waiting, as the two
