 ...
 *
 * TWO shots, chosen for what a still image can actually carry:
 *
 * 1. The plugin's own notice after a rename, with the rewritten link on screen behind it. A rename is a
 ...
 * 2. The refusal notice, with a plugin installed that still owns its own rename/delete handler. It is
 *    the first thing a user with one of those installed will see, so it belongs in the listing rather
 ...
 *
 * **The settings tab is deliberately not shown, and it was the first choice.** `app.setting.open()`
 * never opens the modal in the hidden capture window — measured, a 20 s timeout waiting for
 * `.modal-container`, twice — which matches how modals behave in this harness generally. Both shots
 * here are notices for that reason. A settings frame for the listing has to be taken by hand.
 *
 * There is deliberately no "before and after" pair either. The interesting half of this plugin is what
 * does NOT happen — a link that never broke, an attachment that never stranded — and a frame of a vault
 * that looks correct proves nothing on its own.
 */
...
describe('desktop store screenshots', () => {
  it('1 - the links it rewrote when a note was renamed', async () => {
    ...
    await shoot(1, 'Rename a note and every link to it follows');
  });

  it('2 - what it does when another plugin already owns renames', async () => {
    const probe = await installConflictAndReload();

    expect(probe.noticeText).toContain('Not running');
    expect(probe.isLoaded).toBe(false);
    await shoot(2, 'One owner per vault, and it says so rather than fighting');
  });
});
