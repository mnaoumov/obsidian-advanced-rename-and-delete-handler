...
const CUSTOM_ATTACHMENT_LOCATION_ID = 'obsidian-custom-attachment-location';
const CONSISTENT_ATTACHMENTS_AND_LINKS_ID = 'consistent-attachments-and-links';
...
const ALL_CONFLICTING_IDS = [
  CUSTOM_ATTACHMENT_LOCATION_ID,
  CONSISTENT_ATTACHMENTS_AND_LINKS_ID,
...
describe('findInstalledConflicts', () => {
  ...
  it('should find nothing when every installed version is new enough', () => {
    const app = createApp([
      { pluginId: CUSTOM_ATTACHMENT_LOCATION_ID, version: '12.0.0' },
      { pluginId: CONSISTENT_ATTACHMENTS_AND_LINKS_ID, version: '4.1.2' }
    ]);
  ...
  it('should report a plugin installed below its minimum version', () => {
    const app = createApp([{ pluginId: CUSTOM_ATTACHMENT_LOCATION_ID, version: '11.10.0' }]);

    const conflicts = findInstalledConflicts(app);

    expect(conflicts).toHaveLength(1);
    expect(conflicts[0]?.plugin.pluginId).toBe(CUSTOM_ATTACHMENT_LOCATION_ID);
    expect(conflicts[0]?.installedVersion).toBe('11.10.0');
  });

  it('should report every conflicting plugin, in declaration order', () => {
    const app = createApp([
      { pluginId: CONSISTENT_ATTACHMENTS_AND_LINKS_ID, version: '3.35.4' },
      { pluginId: CUSTOM_ATTACHMENT_LOCATION_ID, version: '11.10.0' }
    ]);

    expect(findInstalledConflicts(app).map((conflict) => conflict.plugin.pluginId)).toEqual([CUSTOM_ATTACHMENT_LOCATION_ID, CONSISTENT_ATTACHMENTS_AND_LINKS_ID]);
  });

  it('should ignore an installed but disabled plugin', () => {
    const app = createApp([{ isEnabled: false, pluginId: CUSTOM_ATTACHMENT_LOCATION_ID, version: '11.10.0' }]);

    expect(findInstalledConflicts(app)).toEqual([]);
  });

  it('should treat an unparseable version as conflicting', () => {
    const app = createApp([{ pluginId: CUSTOM_ATTACHMENT_LOCATION_ID, version: 'not-a-version' }]);
  ...
  it('should report an enabled plugin whose manifest is missing as an unknown version', () => {
    const app = strictProxy<AppOriginal>({
      plugins: strictProxy<AppOriginal['plugins']>({
        enabledPlugins: new Set([CUSTOM_ATTACHMENT_LOCATION_ID]),
  ...
  it('should accept a prerelease of the minimum version as still too old', () => {
    const app = createApp([{ pluginId: CUSTOM_ATTACHMENT_LOCATION_ID, version: '12.0.0-beta.1' }]);
